...
############################################
# Binome : COLLE MAXIME , DOUCHET Benjamin #
############################################
...
# Pour le sudoku, les variables représentent des triplets (i,j,k) avec i, j et k 
# dans [0,8].
...
def encode(i,j,k):
    """La fonction encode prend un triplet (i,j,k) de pos en argument et 
    renvoie un nombre qui indique la variable correspondant à ce triplet.
    """
    return 1 + i + j* 9 + k * 81

def decode(n):
    """Decode prend en argument prend un nombre entre 1 et 729 qui représente 
    une variable représentant un triplet (i,j,k) et renvoie le triplet 
    correspondant.
    """
    m = n-1
    i = m % 9
...
def carre(i1,j1,i2,j2):
    ...
    if i1 == i2 or j1 == j2 :
        res = True
    #on calcule les coord du coin superieur gauche du carré auquel appartiennent les coordonées
    testi1 = i1//3
    testi2 = i2//3
    testj1 = j1//3
    testj2 = j2//3
    if testi1 == testi2 and testj1 == testj2 : 
        return True
    else :
        return False
...
phi5 = [[-encode(i1,j1,k),-encode(i2,j2,k)]
        for (i1,j1,k) in pos 
        for (i2 ,j2,x) in pos 
        if carre(i1,j1,i2,j2)==True and i1!=i2 and j1!=j2]

"""
[-encode(i1,j1,k1),-encode(i2,j2,k2)]
        for (i1,j1,k1) in pos 
        for (i2,j2,k2) in pos
...
[encode(1,1,2)],[encode(1,5,6)],[encode(1,8,8)],
