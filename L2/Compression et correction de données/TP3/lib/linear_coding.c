...
binary_matrix transposed_control_matrix(binary_matrix generator) {
  int i;
  int j;
  int colA = (nb_columns(generator)-nb_rows(generator));
  binary_matrix g = zero_matrix(nb_rows(generator)+colA ,colA);
  
  
  /* on commence par les k premières lignes */ 
  for(i=0 ; i<nb_rows(generator);i++){
    for(j=0;j<nb_columns(g);j++){
      set_matrix_value(g, i, j, matrix_value(generator,i,j+nb_rows(generator)));
    }
  }
  
  /* puis on viens accoler la matrice identité correspondtante */

  for(i=nb_rows(generator);i<nb_rows(g);i++){
    j = i-nb_rows(generator);
...
binary_matrix syndrome(binary_matrix word, binary_matrix tH) {
  ...
  int col , res , ite ;
  binary_matrix synd = zero_matrix(1,tH->columns);
  
  for(col = 0 ; col<tH->columns ; col ++){
    
    res = 0 ;
    for(ite=0 ; ite<tH->rows ; ite ++){
      if(matrix_value(word,0,ite) != matrix_value(tH,ite,col)){
        if(res == 0 ){
          res = 1;
        }
        else{
    ...
    set_matrix_value(synd,0,col,res);
  }
