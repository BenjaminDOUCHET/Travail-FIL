...
huffman_tree *build_huffman_tree(huffman_tree *leaves[], int size) {
  ...
  while (nb_leaves > 0)
  {
    ...
    min_nodes min = get_min_nodes(leaves,
                                  ...
                                  pos_nodes,
                                  nb_nodes);
    //on mets à jour les infos de progression
    pos_nodes += min.nb_nodes;
    nb_nodes -= min.nb_nodes;
    nb_leaves -= min.nb_leaves;

    // on vas créer un nouveau noeud :
    ...
    // maintenant on l'ajoute à nodes
    nodes[pos_nodes + nb_nodes] = new_node;
    //on incrémente le nobre de noeuds
    nb_nodes++;
  }

  return nodes[pos_nodes+nb_nodes-1]; // je dois faire -1 car dans la dernière itération nb_nodes gardera son 1 de trop
}
...
void huffman_decode_file(FILE *input, FILE *output, huffman_tree *tree, int size)
{
  ...
  while (i < size && c != -1)
  {
    ...
    while (!is_huffman_leaf(forest) && max > -1)
    {
      ...
      if (((c >> max) & 1) == 0)
      { /*si le bit observé est à 0 je pars à gauche */      
        forest = forest->left;
      }
      else
      {         
        /* sinon je pars à droite */
        forest = forest->right;
      }
      max--;
    }

    /*à ce niveau si je suis une leaf */
    ...
    {      
      ...
      /*j'écris mon symbole dans le fichier*/
      /* a chaque fois qu'on a trouvé une lettre  il faut repartir du début de l'arbre*/
      forest = tree;
      i++; /*j'incremente le nombnre de symboles écris */ 
    }
    else{
    /*sinon je passe à l'octet suivant*/
...
void huffman_decoding(char *in_filename, char *out_filename, int verbose){
  ...
  /* on déclare les outils */ 
  int counts[ALPHABET_SIZE];
  int i;
  int nb_leaves;
  int total;
   /* on déclare l'arbre d'huffman */ 
  huffman_tree *forest[ALPHABET_SIZE];
  huffman_tree *huff_tree;
  ...
  /* on compte le nombre total de lettre à décoder */ 
  for(i=0 ; i<ALPHABET_SIZE;i++){
      total += counts[i];
  }
